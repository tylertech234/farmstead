<#
.SYNOPSIS
    Turnkey bootstrap for the Farmstead Docker Compose stack.
...
if (-not (Test-Path "docker-compose.yml")) {
    Write-Host "ERROR: Run this script from the Farmstead repository root." -ForegroundColor Red
    exit 1
}
...
$signupBody = @{name="Admin"; email="admin@farmstead.local"; password="Farmstead2026!"} | ConvertTo-Json -Compress
try {
...
- **Email:** ``admin@farmstead.local``
- **Password:** ``Farmstead2026!``
...
| **Open WebUI** | http://chat.localhost | ``admin@farmstead.local`` / ``Farmstead2026!`` |
| **Vikunja** | http://tasks.localhost | ``admin@farmstead.local`` / ``Farmstead2026!`` |
...
Write-Host ""
Write-Host "  Login:          admin@farmstead.local / Farmstead2026!" -ForegroundColor White
